  ...
  recovery: {
    ...
    steps: [
      ...
      },
      {
        ...
        actions: ['等待重跑状态变为“成功”', '打开采集明细，抽查标题、发布单位、公告类型和详情链接', '必要时前往原文库核对正文与来源链路'],
    ...
    steps: [
      {
        ...
        title: '导入网站并创建受控分析批次',
        description: '网站管理支持只导入资产而不触发 AI；需要分析时进入 AI 分析创建批次。可以粘贴 URL、导入 CSV 或 XLSX，也可以选择已有网站。系统按规范化入口 URL 去重，同域名的不同路径分别管理；同一批次最多并发分析 20 个，其余任务保持排队。',
        actions: ['每行填写一个 URL，或使用“网站名称,URL”格式', '批量文件使用网站名称、网站 URL 两列，并选择归属文件夹', '创建后查看运行数和排队数；需要让出 AI 资源时暂停批次，稍后再继续'],
        checkpoint: '完成标志：新 URL 已新增，重复 URL 已识别且未重复建档；分析批次已创建，超出并发上限的任务处于“排队中”。',
        image: null,
        imageAlt: '',
        caption: '',
      },
